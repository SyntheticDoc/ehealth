package ehealth.group1.backend.jely.JELYmaster.de.fau.mad.jely.util;
...
public class Matrix implements Cloneable, java.io.Serializable {
   ...
   */
    ...
    /** Construct a diagonal Matrix from the given arrays of doubles */
    public static Matrix createTridiagonalMatrix(int m, int n, double [] h1h2, double [] h) {
        Matrix mat = new Matrix(m, n, 0);

        for (int i = 0; i < m; i++) {
            if (i == 0) {
                mat.set(i, i, h[i + 1]);
                mat.set(i, i + 1, -h1h2[i]);
                mat.set(i, i + 2, h[i]);
            } else if (i == m - 1) {
                mat.set(i, i, h[i - 2]);
                mat.set(i, i - 1, -h1h2[i - 2]);
                mat.set(i, i - 2, h[i - 1]);
            } else {
                mat.set(i, i, h1h2[i - 1] * 2);
                mat.set(i, i - 1, h[i - 1]);
                mat.set(i, i + 1, h[i]);
            }
        }
